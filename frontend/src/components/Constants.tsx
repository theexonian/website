// Format: First Name (space) Last Name
// Example: Roxane Park
export const EIC_NAME = "Jinmin Lee"
// Format: (apostrophe) Year of Graduation
// Example: '25
export const EIC_GRAD_YEAR = "'26"
// Format: Email
// Example: rhpark@exeter.edu
export const EIC_EMAIL = "jlee22@exeter.edu"
// Format: Board Number (and suffix)
// Example: 146th
export const BOARD_NUMBER = "147th"
// Format: Public-facing AWS IPv4 address
// Example: 34.227.161.14
export const STRAPI_IP = "server.theexonian.net"
// export const STRAPI_IP = "localhost"
export const AWS_IP = "d2stzhv1hip58f.cloudfront.net"